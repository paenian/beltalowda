...
y_beam_inset = 9;
y_motor_offset = [-motor_w/2-linear_rail_width/2, motor_w/2, 0];
y_pulley_offset = y_motor_offset[0];
//The Motor is dependent on the beam - this is an offset from the beam.
y_bracket_hole_lift = linear_rail_carriage_height+11.5;
...
if(part == 2){
    projection(){
        rotate([0,90,0]) translate([-length/2+beam,0,beam/2+.1]) y_plate(front = false);
        rotate([0,-90,0]) translate([length/2-beam,0,beam/2+.1]) y_plate(front = true);
    }
}
...
module y_tensioner(length = 37){
    ...
    difference(){
        union(){
            ...
            //belt pulley mount
            translate([y_pulley_offset,0,length/2]) rotate([90,0,0]) {
                translate([0,0,linear_rail_height/2+wall/2]) cylinder(r1=linear_rail_width/2+wall, r2=m5_rad+1, h=wall+bracket_thick);
                hull(){
                    cylinder(r=linear_rail_width/2+wall, h=linear_rail_height+wall, center=true);
                    cylinder(r=linear_rail_width/2+wall/2, h=linear_rail_height+wall*2, center=true);
                    
                    translate([-y_pulley_offset,-length+linear_rail_width/2,0]) {
                        cylinder(r=linear_rail_width/2+wall, h=linear_rail_height+wall, center=true);
                        cylinder(r=linear_rail_width/2+wall/2, h=linear_rail_height+wall*2, center=true);
                    }
                }
        ...
        //belt nut and shaft
        translate([y_pulley_offset,-linear_rail_height/2+wall*2,length/2]) rotate([90,0,0]){
            cylinder(r1=m5_nut_rad+.25, r2=m5_nut_rad, h=m5_nut_height+1, $fn=6);
...
module y_plate(front = false){
    ...
    difference(){
        union(){
            hull(){
                translate(x_rail_offset_rear+[0,0,beam*1.5+linear_rail_height/2+rad]) rotate([0,90,0]) cylinder(r=rad, h=bracket_thick, center=true);
                ...
                translate(y_beam_offset) rotate([0,90,0]) rotate([0,0,angle]) translate(y_motor_offset) motor_body(extra=1, height=bracket_thick);
                
                translate(extruder_offset) rotate([0,90,0]) rotate([0,0,angle]) translate(y_motor_offset) motor_body(extra=1, height=bracket_thick);
            }
        }
    ...
    difference(){
        union(){
            ...
            mirror([0,0,0]) translate([0,y_pulley_offset+pulley_rad+1,bracket_thick]) hull(){
                ...
                translate([0,wall,-bracket_thick]) cube([hole_sep_x+hole_rad*2+wall,wall*3,bracket_thick],center=true);
        ...
        //belt grip
        for(i=[1]) translate([0,y_pulley_offset+pulley_rad+1,bracket_thick]){
            difference(){
        ...
        for(i=[0,1]) mirror([i,0,0]) translate([hole_sep_x/2+3,y_pulley_offset+4.5,4.5]) rotate([0,90,0]) rotate_extrude(){
    ...
    if(solid){
        ...
        translate([0,0,-inset+4.25+6/2-slop]){
            for(i=[0,1]) mirror([i,0,0]) translate([screw_mount_screw_sep/2,7,0]) rotate([90,0,0]) {
                ...
                //nuts
                translate([0,0,-wall]) rotate([0,0,45]) cylinder(r2=m3_sq_nut_rad, r1=m3_sq_nut_rad+.5, h=m3_nut_height+1, center=true, $fn=4);
                hull(){
                    translate([0,m3_sq_nut_rad/2,-wall]) rotate([0,0,45]) cylinder(r2=m3_sq_nut_rad, r1=m3_sq_nut_rad+.5, h=m3_nut_height+1, center=true, $fn=4);
                    translate([0,-m3_sq_nut_rad*2,-wall]) rotate([0,0,45]) cylinder(r2=m3_sq_nut_rad+.5, r1=m3_sq_nut_rad+1, h=m3_nut_height+1, center=true, $fn=4);
                }
